...
/*
make_instr_func(mov_cr_r32)
{
	OPERAND dest;
	dest.data_size = data_size;
	int len = modrm_rm(eip+1,&dest);
...
make_instr_func(mov_rm16_sreg)
{
	...
	OPERAND r , sreg;
	r.data_size = data_size;
	sreg.data_size = data_size;

	len += modrm_r_rm(eip+1, &sreg, &r);
	...
	//printf("eip = %x\n",eip);
	//uint32_t temp = opcode & 0x7;
	//printf("reg = %x\n",temp);

	print_asm_2("mov", "", len, &r, &sreg);
	return len;
...
//	printf("sreg.addr = %x\n",dest.addr);
	...
	sreg.type = OPR_SREG;
	cpu.segReg[sreg.addr].val = src.val;
	load_sreg(sreg.addr);
