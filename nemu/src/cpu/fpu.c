...
uint64_t myssr(uint64_t sig_grs)
{	
	uint64_t first=sig_grs&0x1;
	sig_grs>>=1;
	uint64_t second=sig_grs&0x1;
...
inline uint32_t internal_normalize(uint32_t sign, int32_t exp, uint64_t sig_grs) {
	...
	if((sig_grs >> (23 + 3)) > 1 || exp < 0) {
		...
		if(exp < 0) { 
			/* TOD	O: assign the number to zero */
			//printf("\e[0;31mPlease implement me at fpu.c\e[0m\n");
			//assert(0);
			exp=0;
			sig_grs=0;
	...
	if(!overflow) {
		...
		//printf("\e[0;31mPlease implement me at fpu.c\e[0m\n");
		//assert(0);
		uint64_t grsb=sig_grs&0x7;//round up
		//printf("%llx\n",grsb);
		if(grsb>4)
		{
			sig_grs>>=3;
		...
		//more normalize
		while(sig_grs>0xffffff)
		{
			exp++;
			//uint64_t temp=0x1&sig_grs;//save the stick bit
			sig_grs>>=1;
			/*if(temp==0)
				sig_grs&=0xFFFFFFE;
			e	lse
...
uint32_t internal_float_add(uint32_t b, uint32_t a) {
	...
	// alignment shift for fauint32_t sticky = 0;
